...
class CodexRuntimeBuilder:
    ...
    def build(self) -> CodexRuntime:
        ...
        ``MALA_LOCK_DIR``, ``MALA_REPO_NAMESPACE``) and exempt scratch
        validation logs (``MALA_VALIDATION_LOG_DIR``). ``MALA_LOCK_DIR``
        and ``MALA_VALIDATION_LOG_DIR`` are resolved through
        :mod:`src.infra.tools.env` rather than relying on the parent
        ``os.environ``: the hook reads its env strictly from what the
        Codex subprocess inherits, and an unset ``MALA_LOCK_DIR`` in the
        orchestrator's env would cause the hook to deny every lock-gated
        shell/apply_patch write fail-closed. Mirrors the Amp path's
        posture at ``src/infra/clients/amp_runtime.py:288``.
        ...
        from src.infra.tools.env import get_lock_dir, get_repo_validation_log_dir
