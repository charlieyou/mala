...
a DISABLED sentinel. When a command is set to DISABLED, it means the user
explicitly wants to disable that command even if the preset defines it.
"""
...
class _DisabledSentinel:
    """Sentinel value indicating a command is explicitly disabled.
...
"""Sentinel value for explicitly disabled commands.

Use this when parsing user config where a command is explicitly set to null,
...
def merge_configs(
    ...
    explicitly set to DISABLED sentinel, the command is disabled even if
    ...
    # Coverage: user replaces entirely if present
...
def _merge_coverage(
    ...
    User coverage replaces preset coverage entirely if present.
    There's no field-level merging for coverage.
    """
    # If user specified coverage (including explicit None to disable), use it
    # We check if user_cov is not None, meaning user provided coverage config
