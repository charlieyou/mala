...
coder. Coder-specific settings only apply to their matching coder:

| Flag/setting | `coder=claude` | `coder=amp` | `coder=codex` |
|--------------|----------------|-------------|---------------|
| `--claude-settings-sources` / `MALA_CLAUDE_SETTINGS_SOURCES` | applied | ignored | ignored |
| `--amp-mode` / `MALA_AMP_MODE` / `amp_mode` | ignored | applied | ignored |
| `--codex-*` / `MALA_CODEX_*` / `coder_options.codex.*` | ignored | ignored | applied |
...
`mala-safety` plugin only inside mala-launched Codex sessions, and remediation
steps for each fail-closed
`CodexHookNotActiveError` / `CodexNotInstalledError` reason. The error class's
`docs_url` attribute points at this section.

### Scoped auto-trust (default)
...
### Legacy global install cleanup

Older mala builds installed `mala-safety` into the user's real Codex home. In a
normal Codex CLI session the `MALA_*` lock context is absent, so that global
hook can fail closed and block edits. Current mala releases no longer write the
plugin globally, but they also do not delete user config automatically. If your
ordinary Codex sessions are still blocked from edits, clean up the legacy global
install in your normal Codex home:

1. Delete the cached plugin tree:

   ```bash
   rm -rf "${CODEX_HOME:-$HOME/.codex}/plugins/cache/local/mala-safety"
   ```

2. Remove or disable the legacy `mala-safety` tables from
   `${CODEX_HOME:-$HOME/.codex}/config.toml` if they are present:
   ...
   Deleting those blocks is sufficient. If you prefer not to delete them, set
   the plugin table's `enabled = false`. Review any `[features] hooks`,
   `plugins`, or `plugin_hooks` lines if you had previously kept those disabled
   for normal Codex; older mala versions may have flipped them to `true`.
