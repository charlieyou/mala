...
- **In Scope**
  ...
  - R4: Cerberus implementation using `spawn-epic-review` command
...
- **Out of Scope (Non-Goals)**
  ...
  - Implementation of `spawn-epic-review` Cerberus command (external dependency)

## Assumptions & Constraints
- Cerberus `spawn-epic-review` command follows the same spawn/wait pattern as `review-gate`
- `spawn-epic-review` output JSON structure is compatible with `EpicVerdict`
...
- Cerberus subcommand detection: Run `spawn-epic-review --help` and require exit 0 (per spec R5)

### Testing Constraints
- Mock Cerberus subprocess for unit tests (no real subprocess in CI)
- No integration tests with real Cerberus until plugin ships `spawn-epic-review`
...
- [ ] Cerberus plugin `spawn-epic-review` command exists (external dependency - can proceed without, will be disabled)
...
1. Create `src/infra/clients/cerberus_epic_verifier.py` with `CerberusEpicVerifier` class
   ...
   - Spawn-wait pattern: `spawn-epic-review --context-json <path>` then poll/wait
   - Parse JSON response to `EpicVerdict`
...
1. Add `_check_epic_verifier_availability()` to `src/orchestration/factory.py`
   ...
   - For `cerberus`: probe `spawn-epic-review --help`, require exit 0
...
EpicVerifier.verify_and_close_epic()
  ...
  └─> CerberusEpicVerifier.verify()
        ...
        └─> spawn: spawn-epic-review --context-json <path>
        └─> wait: poll for result file or process completion
        └─> parse JSON response → EpicVerdict
...
**Cerberus Context JSON** (passed to `spawn-epic-review`):
```json
{
  "epic_id": "string",
  "acceptance_criteria": "string (markdown)",
  "spec_content": "string (spec file content, if available)",
  "commit_range": {"start": "sha", "end": "sha"},
...
### Risks
- **Cerberus plugin not updated**: If `spawn-epic-review` command doesn't exist in the plugin, users configuring `reviewer_type: cerberus` get `cerberus_unavailable` error at startup. Mitigation: Clear error message with instructions.
...
- **Cerberus binary exists but subcommand missing**: `spawn-epic-review --help` returns non-zero. Fail with actionable error: "cerberus plugin detected but spawn-epic-review subcommand unavailable".
...
2. Run with Cerberus plugin installed → verify `spawn-epic-review` invoked correctly
...
1. ~~**Cerberus subcommand detection**~~ → RESOLVED: Run `spawn-epic-review --help` and require exit 0 (per spec R5)
...
6. **Context JSON schema versioning**: Should the context JSON passed to `spawn-epic-review` include a schema version field for forward compatibility? Suggest adding `"schema_version": "1"` to the payload.
