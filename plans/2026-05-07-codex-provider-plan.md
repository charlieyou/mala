...
- **`Thread.read(include_turns=True)` viability for evidence (Phase F1 spike)** — **RESOLVED: DISCONFIRMED, switching to F3 tee fallback for T013** (recorded by T012, 2026-05-08). Source-review evidence (Codex SDK local checkout at `~/code/codex/codex-rs/`):
  ...
  - Spike artifact: `tests/spike/test_codex_thread_read_evidence.py` — runtime assertions match the lossless ideal so a future Codex SDK release that re-enables Extended persistence (or otherwise surfaces aggregated output through `thread/read`) would flip the tests to pass without rewriting the spike. Skips when `codex_app_server` / `codex` binary / auth are absent (current local + CI state).
