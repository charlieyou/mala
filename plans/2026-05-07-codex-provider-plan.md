...
**Concurrency invariant (per-process env isolation).** Mala MUST pass the `MALA_*` vars to the Codex subprocess via an explicit per-process env dict, NOT by mutating `os.environ` in the parent — parity with the Amp path (`AmpRuntimeBuilder.build()` constructs `env: dict[str, str] = {**os.environ, ...overlays}` explicitly, `src/infra/clients/amp_runtime.py:315-327`). Otherwise, when `--max-agents > 1` is in effect, concurrent Codex agents would clobber each other's `MALA_AGENT_ID` in the shared parent `os.environ` and the inherited subprocess env would identify the wrong owner to the bundled `PreToolUse` hook. The Phase B/C spike confirmed `codex_app_server.AppServerConfig.env` accepts an explicit per-process env dict, so `CodexRuntimeBuilder.build()` overlays the full identity bundle (`MALA_AGENT_ID` + `MALA_LOCK_DIR` resolved via `src.infra.tools.env.get_lock_dir` + `MALA_REPO_NAMESPACE`) onto that dict — this is the sole supported transport for the hook env contract.

The hook reads the bundle strictly from `os.environ` (no on-disk fallback); a missing key fail-closes with `permissionDecisionReason: "MALA_AGENT_ID/MALA_LOCK_DIR/MALA_REPO_NAMESPACE missing; refusing to evaluate lock"`. A historical state-file fallback (`~/.config/mala/agent-state/{session_id}.env`) was specified as a contingency before the spike; it has been removed now that env injection is confirmed working (no producer was ever wired and the read path was dead defensive code; bd-mala-b18dd.9.6).
...
- **`Thread.read(include_turns=True)` viability for evidence (Phase F1 spike)** — **RESOLVED: DISCONFIRMED, switching to F3 tee fallback for T013** (recorded by T012, 2026-05-08). Source-review evidence (Codex SDK local checkout at `~/code/codex/codex-rs/`):
  - `app-server/src/request_processors/thread_processor.rs:5-8` declares `"persistExtendedHistory is deprecated and ignored. App-server always uses limited history persistence."` — passing `persistExtendedHistory: true` triggers a `DeprecationNotice` and is otherwise a no-op.
  - `app-server/src/request_processors.rs:503-511` (`build_api_turns_from_rollout_items`, the function `thread/read` uses) hardcodes `EventPersistenceMode::Limited` for the rollout-item filter.
  - `rollout/src/policy.rs:111-131` (`event_msg_persistence_mode`) lists `EventMsg::ExecCommandEnd` under `EventPersistenceMode::Extended` only. Combined with the rollout writer at `rollout/src/recorder.rs:797-821` which drops items the policy rejects, `ExecCommandEnd` is **never written to the rollout** in app-server runs.
  - `app-server-protocol/src/protocol/item_builders.rs:88-133` builds `CommandExecutionThreadItem.aggregated_output` *only* from `ExecCommandEndEvent.aggregated_output`. The Begin event sets it to `None`. With End events filtered, replay yields `aggregated_output: None` for every completed bash turn — i.e. lint/build evidence is unobservable through `Thread.read(include_turns=True)`.
  - Cross-resume: `ThreadRollbackResponse.thread` schema docstring (`generated/v2_all.py:8336-8341`) states: *"The ThreadItems stored in each Turn are lossy since we explicitly do not persist all agent interactions, such as command executions. This is the same behavior as `thread/resume`."* `thread/read` reads the same rollout, so the loss is shared.
  - Spike findings (acceptance-criteria mapping): (1) `aggregated_output` present? **No** (always `None` for `CommandExecution` items) — fails AC. (2) per-call read latency? **Not measured** without the runtime; the source path is single-rollout-file IO with no network and no pagination, so cost is bounded by rollout size, but this becomes moot since (1) already disqualifies the path. (3) cross-resume completeness? **No** (lossy by upstream design, per schema docstring) — fails AC. File-edit coverage: `EventMsg::PatchApplyEnd` *is* in the Limited persistence set (`rollout/src/policy.rs:99`), so `FileChangeThreadItem.changes` *should* survive `Thread.read`; the spike asserts this directly because it's a separate invariant from the bash-aggregation gap that drives the F3 fallback decision.
  - Spike artifact: `tests/spike/test_codex_thread_read_evidence.py` — three runtime tests, marker `integration` so the issue verification command (`uv run pytest tests/spike/test_codex_thread_read_evidence.py`) collects them under the repo's default `-m 'unit or integration'` filter; module-level `pytest.importorskip("codex_app_server")` + a `codex` binary probe skip the whole module when the SDK / runtime / auth aren't present (current local + CI state). Driven through the Thread handle: `codex.thread_start()` → `thread.run(input=...)` for both bash and `apply_patch` file edits, and `codex.thread_resume(thread_id)` → `resumed.run(...)` / `resumed.read(...)` (the SDK returns a `Thread`, not a response wrapper). Assertions match the lossless ideal so a future Codex SDK release that re-enables Extended persistence (or otherwise surfaces aggregated output through `thread/read`) flips the tests green without rewriting the spike.
  - **Action — T013 implements F3 tee fallback** at `~/.config/mala/codex-sessions/{thread_id}.jsonl` (decision #11 reversed). The plan F3 contingency becomes the primary path; F2 native is dropped. T012 spike test stays in `tests/spike/` so a future SDK change can be validated cheaply (see also Phase F1 detail and `Phase F` test list — the contingency-tee test is now mandatory, not optional).
