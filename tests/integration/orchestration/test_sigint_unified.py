...
"""
    ...
    def test_validation_ran_true_when_issues_completed(self, tmp_path: Path) -> None:
        """validation_ran=True when validation actually runs.

        When issues complete successfully and validation runs,
        validation_ran should be True.
        """
        script = tmp_path / "validation_ran_test.py"
        script.write_text(
            """
import asyncio
import sys
...
async def main():
    ...
    # Mock run_implementer to return success immediately
    orchestrator.run_implementer = mock_agent

    # Mock run_validation to return success
    from src.pipeline.run_coordinator import GlobalValidationOutput

    async def mock_validation(validation_input, *, interrupt_event=None):
        return GlobalValidationOutput(passed=True, interrupted=False)

    orchestrator.run_coordinator.run_validation = mock_validation
    ...
    # Print result for test verification
    if event_sink.validation_ran is True:
        print("VALIDATION_RAN_TRUE", flush=True)
    elif event_sink.validation_ran is False:
        print("VALIDATION_RAN_FALSE", flush=True)
    else:
        print(f"VALIDATION_RAN_UNKNOWN={event_sink.validation_ran}", flush=True)
...
"""
        ...
        try:
            ...
            # Should exit 0 (validation passed)
            assert proc.returncode == 0, (
                f"Expected exit code 0, got {proc.returncode}. "
                f"Stdout: {stdout}\nStderr: {stderr}"
            )
            ...
            # validation_ran should be True
            assert "VALIDATION_RAN_TRUE" in stdout, (
                f"Expected VALIDATION_RAN_TRUE in output. "
                f"Stdout: {stdout}\nStderr: {stderr}"
            )
