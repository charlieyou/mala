...
class TestIdleTimeoutResumeIntegration:
    """Integration tests for idle timeout resume wiring in AgentSessionRunner.

    These tests verify the end-to-end behavior of idle timeout handling:
    - State reset on timeout (tool_calls_this_turn, first_message_received, pending_tool_ids)
    - Query retry using session_id from previous attempt
    - Backoff delays between retry attempts

    Unlike unit tests that mock internal components, these tests drive
    AgentSessionRunner with fake SDK clients to verify the full wiring.
    """

    @pytest.fixture
    def tmp_log_path(self, tmp_path: Path) -> Path:
        """Create a temporary log file that exists."""
        log_path = tmp_path / "session.jsonl"
        log_path.write_text("")
        return log_path

    @pytest.mark.asyncio
    async def test_idle_timeout_resume_verifies_session_id_and_state_reset(
        self,
        tmp_path: Path,
        tmp_log_path: Path,
    ) -> None:
        """Verify idle timeout resume uses session_id and resets state correctly.

        This integration test verifies the end-to-end contract:
        1. First attempt yields session_id via ResultMessage partial, then hangs
        2. On timeout, state is reset (tool_calls=0, first_message=False, pending_tool_ids empty)
        3. Retry uses resume prompt with session_id from previous attempt
        4. Successful retry completes the session

        This is the key behavioral contract for idle timeout resume:
        - Session continuity via session_id
        - Fresh state for each attempt
        """
        # Capture session_id from first attempt to verify it's used in retry
        captured_session_id = "hang-session-for-resume"

        # First client: yields ResultMessage with session_id, then hangs
        # The ResultMessage partial provides session_id before stream completion
        hanging_result = make_result_message(session_id=captured_session_id)
        hanging_client = HangingAfterMessagesSDKClient(messages=[hanging_result])
        ...
        session_config = AgentSessionConfig(
            ...
            timeout_seconds=60,
            prompts=make_test_prompts(),
            idle_timeout_seconds=0.01,  # Very short for fast test
            max_idle_retries=2,
            idle_retry_backoff=(0.0, 0.0, 0.0),  # No delays for test speed
            review_enabled=False,
        )
        ...
        input_data = AgentSessionInput(
            issue_id="resume-test-123",
            prompt="Initial prompt for resume test",
        )

        output = await runner.run_session(input_data)

        # === Verify success after retry ===
        assert output.success is True, f"Expected success, got: {output.summary}"

        # === Verify two clients were created (original + 1 retry) ===
        assert len(factory.create_calls) == 2, (
            f"Expected 2 clients (original + retry), got {len(factory.create_calls)}"
        )

        # === Verify disconnect was called on hanging client ===
        assert hanging_client.disconnect_called is True, (
            "disconnect() must be called on timed-out client"
        )

        # === Verify retry used session_id from first attempt ===
        assert len(success_client.queries) == 1, (
            f"Expected 1 query to success client, got {len(success_client.queries)}"
        )
        retry_prompt, retry_session_id = success_client.queries[0]

        # Session ID from first attempt must be passed to retry
        assert retry_session_id == captured_session_id, (
            f"Expected session_id={captured_session_id}, got {retry_session_id}"
        )

        # Resume prompt must be used (not original prompt)
        assert "Continue on issue resume-test-123" in retry_prompt, (
            f"Expected resume prompt, got: {retry_prompt}"
        )

    @pytest.mark.asyncio
    async def test_idle_timeout_resume_with_backoff_timing(
        self,
        tmp_path: Path,
        tmp_log_path: Path,
    ) -> None:
        """Verify backoff delays are applied between idle retry attempts.

        This test verifies:
        1. Retry 1 uses backoff[0] (0s - no sleep)
        2. Retry 2 uses backoff[1] (recorded as sleep call)
        3. Backoff delays accumulate correctly

        The backoff ensures we don't hammer the API on transient issues.
        """
        session_id = "backoff-test-session"

        # Three clients: first two hang, third succeeds
        ...
        session_config = AgentSessionConfig(
            ...
            idle_timeout_seconds=0.01,
            max_idle_retries=2,
            idle_retry_backoff=(0.0, 0.1, 0.2),  # 0s, 0.1s, 0.2s
            review_enabled=False,
        )
        ...
        input_data = AgentSessionInput(
            issue_id="backoff-test",
            prompt="Test backoff timing",
        )
        ...
        async def mock_sleep(delay: float) -> None:
            sleep_calls.append(delay)
            # Use minimal real sleep to allow test to progress
            await original_sleep(0.001)

        with patch(
            "src.pipeline.idle_retry_policy.asyncio.sleep",
        ...
        # === Verify 3 clients were created ===
        assert len(factory.create_calls) == 3

        # === Verify backoff was applied ===
        # Retry 1: backoff[0] = 0.0 (no sleep call for 0)
        # Retry 2: backoff[1] = 0.1 (should appear in sleep_calls)
        assert 0.1 in sleep_calls, (
            f"Expected 0.1s backoff in sleep calls, got: {sleep_calls}"
        )

    @pytest.mark.asyncio
    async def test_idle_timeout_fresh_session_on_no_side_effects(
        self,
        tmp_path: Path,
        tmp_log_path: Path,
    ) -> None:
        """Verify retry starts fresh session when no side effects occurred.

        When idle timeout happens before any messages are received:
        - No session_id exists
        - No tool calls occurred
        - Retry should use original prompt with no session_id (fresh session)

        This is different from resume behavior where session_id is passed.
        """
        # First client: hangs immediately (no messages, no session_id)
        hanging_client = HangingSDKClient()
        ...
        original_prompt = "Original prompt for fresh session test"
        input_data = AgentSessionInput(
            issue_id="fresh-test",
        ...
        # === Verify retry used ORIGINAL prompt (not resume prompt) ===
        assert len(success_client.queries) == 1
        retry_prompt, retry_session_id = success_client.queries[0]

        assert retry_prompt == original_prompt, (
            f"Expected original prompt, got: {retry_prompt}"
        )
        assert retry_session_id is None, (
            f"Expected no session_id for fresh session, got: {retry_session_id}"
        )
