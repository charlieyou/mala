...
class TestMalaConfigFromEnv:
    ...
    def test_new_cerberus_runtime_fields_default_to_none(self) -> None:
        """Cerberus runtime fields default to None until the factory injects them."""
        config = MalaConfig()
        assert config.cerberus_state_root is None
        assert config.cerberus_project_key is None

    def test_from_env_does_not_populate_cerberus_runtime_fields(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """from_env() leaves runtime-derived Cerberus fields unset."""
        monkeypatch.setenv("CLAUDE_CONFIG_DIR", "/custom/claude")
        config = MalaConfig.from_env(validate=False)
        assert config.cerberus_state_root is None
        assert config.cerberus_project_key is None

    def test_removed_binary_path_kwarg_raises_type_error(self) -> None:
        """The removed binary path field has no compatibility shim."""
        removed_kwarg = "_".join(("cerberus", "bin", "path"))
        with pytest.raises(TypeError):
            MalaConfig(**{removed_kwarg: Path("/tmp/bin")})  # ty:ignore[invalid-argument-type]
